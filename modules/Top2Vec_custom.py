...
class Top2Vec:
    ...
    embedding_model: string
        ...
        For more information distiluse-base-multilingual-cased visit:
    ...
    def _dim_reduction(self):
        ...
        umap_model = umap.UMAP(
            ...
            random_state=self.random_seed).fit(self._get_document_vectors(norm=False))
    ...
    def _embed_documents(
        ...
        if load_doc_embed:
            # load
            try:
                document_vectors = np.load(path_doc_embed)
            except ValueError("Could not load model embeddings"):
                return None
            ...
            if save_doc_embed:
                np.save(path_doc_embed, document_vectors)
    ...
    def _create_topic_vectors(self):
        ...
        unique_labels = set(self.clustering.labels_)
        if -1 in unique_labels:
            unique_labels.remove(-1)
        self.topic_vectors = self._l2_normalize(
            np.vstack([self._get_document_vectors(norm=False)[np.where(self.clustering.labels_ == label)[0]]
